...
const rowsByName=new Map();for(const row of rows){const key=norm(row.name);if(!rowsByName.has(key))rowsByName.set(key,[]);rowsByName.get(key).push(row);}
...
function apply(p,b){p._agencyEvidence=null;const r=(rowsByName.get(norm(p.name))||[]).find(r=>!r.born||!p.born||+r.born===+p.born);if(!r)return;
