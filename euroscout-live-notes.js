...
renderScouting=function(partial=false){
 ...
 const markup=`<div class="view scoutingView liveScouting"><div class="scouthead"><div><h1 class="title">Scouting matchup</h1><div class="sub">Choose a player. Choose a category. Write an observation.</div></div><button class="btn ghost" id="liveSwap">⇄ Swap teams</button></div><div class="liveLayout">${liveColumn(s.a,'a')}<section class="liveNotebook">${p?`<div class="livePlayer"><div class="liveIdentity"><span class="liveAvatar">${photoOf(p)?`<img src="${escAttr(photoOf(p))}" alt="">`:esc(initials(p.name))}</span><div><span class="hint">PLAYER NOTES</span><h2>${esc(p.name)}</h2><div class="liveClub">${esc(liveRoster(p._liveClub)?.t.name||'')}</div></div></div><div class="liveHeaderActions"><button class="btn ghost sm" id="liveProfile">Open full profile ↗</button><button class="btn ghost sm" id="liveNumber">Edit jersey #${esc(shirt(p)||'—')}</button></div></div><div class="liveBody"><div class="liveCategories">${cats.map(([key,label])=>`<button class="btn ${key===category?'primary':'ghost'}" aria-pressed="${key===category}" data-cat="${key}">${label}</button>`).join('')}</div><label for="liveInput">New ${cats.find(c=>c[0]===category)[1].toLowerCase()} observation</label><textarea id="liveInput" rows="4" placeholder="Write what you see…" ${editable()?'':'disabled'}>${esc(draft(p))}</textarea><div class="liveActions"><span class="hint">Enter to add · Shift + Enter for a new line</span><button class="btn primary" id="liveAdd" ${busy||!editable()?'disabled':''}>Add bullet</button></div><p id="liveStatus" role="status">${esc(status||(!editable()?'Sign in with editing access to write notes.':'Saves directly to this player’s profile.'))}</p><details><summary>Viewing log details</summary><label>Game date <input id="liveGameDate" type="date" value="${escAttr(s.gameDate||date)}"></label><label>Watched on <input id="liveDate" type="date" value="${escAttr(s.noteDate||date)}"></label><p class="hint">Saved in Reports → Viewing log when you add a note.</p></details><h3>${cats.find(c=>c[0]===category)[1]} notes</h3><div id="liveBullets"></div></div>`:'<div class="empty">Choose a team, then select a player to begin taking notes.</div>'}</section>${liveColumn(s.b,'b')}</div><div class="liveRecent">${recent.map(id=>players.find(p=>p.id===id)).filter(Boolean).map(p=>`<button class="btn ghost sm" data-recent="${escAttr(p.id)}">${esc(p.name)}</button>`).join('')}</div></div>`;
 ...
 async function add(){if(busy||!editable()||!input.value.trim())return;busy=true;document.querySelector('#liveAdd').disabled=true;
  ...
  r[field]=[r[field],...text.split(/\r?\n/).filter(x=>x.trim()).map(x=>'• '+x.trim())].filter(Boolean).join('\n');
  logViewing(r,game,s.noteDate||date,s.gameDate||date,author);
 ...
 if(editable()){let editor=makeBulletList(host,rep[field]||'','Observation',()=>{const r=parseReport(recOf(p).report);r[field]=editor.serialize();logViewing(r,['a','b'].map(k=>liveRoster(s[k])?.t.name).filter(Boolean).join(' vs '),s.noteDate||date,s.gameDate||date,window.ESAccess?.user?.email||'');saveRec(p,{report:JSON.stringify(r)}).then(ok=>{const el=document.querySelector('#liveStatus');if(el)el.textContent=ok===false?'Stored locally; cloud sync failed.':'Saved to player profile.'})});}
