...
function contextHTML(){const s=STATE.scouting,a=SX?.active(),ra=liveRoster(s.a),rb=liveRoster(s.b);
 ...
 const session=!SX?'':a?'<span class="mx-live"><i></i>Session running</span><button type="button" class="btn primary sm" id="mxFinish">Finish session</button>':(editable()?'<button type="button" class="btn primary sm" id="mxStart">Start session</button>':'');
