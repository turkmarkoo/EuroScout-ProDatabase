...
function contextHTML(){const s=STATE.scouting,a=SX?.active(),ra=liveRoster(s.a),rb=liveRoster(s.b);
 ...
 const session=!SX?'':a?'<span class="mx-live" title="Session running"><i></i><span id="mxClock">00:00:00</span></span><button type="button" class="btn primary sm" id="mxFinish">Finish session</button>':(editable()?'<button type="button" class="btn primary sm" id="mxStart">Start session</button>':'');
...
function headerHTML(p){const lv=levelBand(p),g=statGradeOverall(p),photo=photoOf(p),num=shirt(p),stock=SX?SX.stockOf(p):'',a=SX?.active();
 ...
 return '<div class="livePlayer mx-player"><span class="liveAvatar mx-avatar">'+(photo?'<img src="'+escAttr(photo)+'" alt="">':esc(initials(p.name)))+(num?'<em>#'+esc(num)+'</em>':'')+'</span>'+
  ...
  '<div class="mx-rate">'+(lv?'<div class="mx-level"><small>Estimated level'+(lv.manual?'':' · auto')+'</small><b>'+esc(lv.label)+'</b><span class="mx-segs">'+[0,1,2,3,4].map(i=>'<i class="'+(i<=lv.i?'on':'')+'"></i>').join('')+'</span></div>':'')+(g!=null?'<div class="mx-grade"><small>Grade</small><b>'+Number(g).toFixed(1)+'</b></div>':'')+'</div>'+
...
function tick(){clearInterval(timer);const a=SX?.active();if(!a)return;const paint=()=>{const el=document.querySelector('#mxClock');if(!el){clearInterval(timer);return;}el.textContent=SX.clock((Date.now()-new Date(a.startedAt))/1000);refreshMarks();};paint();timer=setInterval(paint,1000);}
...
function wireNotebook(p,rep){
 ...
 document.querySelector('#mxLogs').onclick=()=>SX?SX.openPlayerLog(p):openProfile(p.id);
