...
openEditModal=function(id){original(id);const box=document.querySelector('#modal .editgrid'),save=document.getElementById('editSave');if(!box||!save)return;
 ...
 const clubs=allClubs(),norm=s=>String(s).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
 ...
 input.oninput=paint;select.onchange=()=>{selected=select.value;choice.textContent='Selected: '+label(selected)+' · saves with Save';};paint();
