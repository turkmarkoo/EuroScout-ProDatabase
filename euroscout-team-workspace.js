(function () {
  ...
  const is2627 = season => /^2026\s*[\/-]\s*27$/.test(String(season||''));
  const leagueMark = L => {
    const id=String(L.meta.id||'').toLowerCase();
    const domains={aba:'aba-liga.com',euroleague:'euroleaguebasketball.net',eurocup:'euroleaguebasketball.net',bcl:'championsleague.basketball',bclq:'championsleague.basketball',nba:'nba.com',gleague:'gleague.nba.com',ncaam:'ncaa.com',acb:'acb.com',lba:'legabasket.it'};
    const logo=L.meta.logo||L.meta.logoUrl||(domains[id]?'https://www.google.com/s2/favicons?domain='+domains[id]+'&sz=128':'');
    return logo
      ? `<img src="${escAttr(logo)}" alt="${escAttr(L.meta.name)} logo" loading="lazy" onerror="this.parentNode.textContent='${escAttr(id.toUpperCase().slice(0,4))}'">`
      : `<span>${esc(id.toUpperCase().slice(0,4))}</span>`;
  };
  ...
  window.renderTeamScoutingV2 = function renderTeamScoutingV2() {
    ...
    const standings=hasStats?standingRows(L,t):[], rank=standings.findIndex(x=>x.code===t.code)+1;
    const wins=hasStats?N(t.w):null,losses=hasStats?N(t.l):null,winPercent=wins!=null&&losses!=null&&wins+losses>0?wins/(wins+losses)*100:null;
    const ckey=clubKey(L,t), watching=watchedClubs().has(ckey);
    const country=t.country?countryLabel(t.country)||t.country:'', plainCountry=t.country&&typeof EuroScoutCountries!=='undefined'?EuroScoutCountries.canonical(t.country):t.country||'', city=t.city||'';
    const logo=t.logo||dbTeamByKey(L.meta.id+'|'+t.code)?.logo||'';
    const cards=hasStats?(mode==='adv'?advanced:basic).map(([k,label,hi])=>{
    ...
    const card=p=>{const src=photo(p);const born=p.born?String(p.born).slice(0,4):'—';const height=N(p.height)?(N(p.height)/100).toFixed(2)+' m':'—';const jersey=p.jersey===0||p.jersey==='0'?'0':p.jersey||'—';
      return `<article class="pcard tswPlayer" data-player="${escAttr(p.id)}" tabindex="0" role="button" aria-label="Open ${escAttr(p.name)} profile"><span class="pc-watch${isWatched(p)?' on':''}" data-star="${escAttr(p.id)}" role="button" tabindex="0" aria-label="${isWatched(p)?'Remove from':'Add to'} watchlist">${isWatched(p)?'★':'☆'}</span><div class="pc-top"><div class="pc-ava">${src?`<img src="${escAttr(src)}" alt="" loading="lazy" onerror="this.remove()">`:esc(initials(p.name))}</div><div class="pc-num">${esc(jersey)}</div><div class="pc-id"><div class="pc-name">${esc(p.name)}</div><div class="pc-meta">${esc(p.pos&&p.pos!=='-'?p.pos:p.role||'—')} · ${height} · '${esc(born.slice(-2))}</div></div></div><div class="pc-stats">${[['PPG',hasStats?p.ppg:null],['RPG',hasStats?p.rpg:null],['APG',hasStats?p.apg:null],['PIR',hasStats?p.pir:null]].map(([label,value])=>`<div class="s"><div class="v">${F(value)}</div><div class="k">${label}</div></div>`).join('')}</div></article>`;};
    $('#app').innerHTML=`<div class="view tswPage">
      ...
      <section class="tswHero"><div class="tswLogo">${logo?`<img src="${escAttr(logo)}" alt="" onerror="this.parentNode.textContent='${escAttr(initials(t.name))}'">`:esc(initials(t.name))}</div><div class="tswIdentity"><div class="tswTitle"><h1>${esc(t.name)}</h1><button type="button" id="tswStar" aria-label="${watching?'Remove club from':'Add club to'} watchlist" aria-pressed="${watching}">${watching?'★':'☆'}</button></div><p>${esc(country)} <span>·</span> ${esc(L.meta.name)} <span>·</span> ${esc(season)}</p>${city?`<small>⌖ &nbsp;${esc(city)}${plainCountry?', '+esc(plainCountry):''}</small>`:''}</div><div class="tswControls"><div class="tswActions"><button class="btn ghost" id="tswCompare" type="button" ${hasStats?'':'disabled title="Current-season comparison is unavailable until results are loaded"'}>⇄ &nbsp;Compare</button><button class="btn ghost" id="tswWatch" type="button">${watching?'★ Watching':'☆ Add to watchlist'}</button></div><div class="tswControlBottom"><label>Competition<select id="tswCompetition">${group.map(g=>`<option value="${escAttr(g.league+'|'+g.code)}"${g.league===L.meta.id&&g.code===t.code?' selected':''}>${esc(g.L.meta.name)}</option>`).join('')}</select></label><label>Season<select id="tswSeason"><option value="2026/27"${current?' selected':''}>2026/27</option><option value="2025/26"${current?'':' selected'}>2025/26</option></select></label><div class="tswRecord"><div><strong>${wins==null||losses==null?'—':wins+'-'+losses}</strong><small>RECORD</small></div><div><strong>${pct(winPercent)}</strong><small>WIN %</small></div><div><strong>${rank||'—'}</strong><small>RANK</small></div></div></div></div></section>
      <main class="tswColumns"><div class="tswLeft"><section class="tswPanel"><div class="tswPanelTitle"><div><h2>Team averages · ${esc(L.meta.name)}</h2><p>${hasStats?`Team rankings compared with ${esc(L.meta.name)} teams (${esc(season)}).`:`${esc(season)} team statistics have not been loaded yet.`}</p></div><div class="tswSegment"><button type="button" data-mode="basic" class="${mode==='basic'?'active':''}">Basic</button><button type="button" data-mode="adv" class="${mode==='adv'?'active':''}">Advanced</button></div></div><div class="tswStats">${cards||'<p class="hint">No current-season results yet. Choose 2025/26 to view historical statistics.</p>'}</div></section>
      <section class="tswPanel tswRoster"><div class="tswPanelTitle"><div><h2>Roster · ${roster.length} players</h2><p>${esc(L.meta.name)} ${esc(season)}${current&&!is2627(L.meta.season)?' · current club assignments':''}</p></div><div class="tswSegment"><button type="button" data-roster-view="grid" class="${view==='grid'?'active':''}">▦ Grid</button><button type="button" data-roster-view="list" class="${view==='list'?'active':''}">☷ List</button></div></div><div class="rostergrid tswRosterGrid ${view==='list'?'list':''}">${roster.length?roster.map(card).join(''):'<p class="hint">No roster recorded for this season.</p>'}</div></section></div>
      <aside class="tswPanel tswStandings"><div class="tswStandHead"><span class="tswLeagueMark">${leagueMark(L)}</span><div><h2>${esc(L.meta.name)}</h2><p>Standings · ${esc(season)}</p></div></div><table><thead><tr><th>#</th><th>Team</th><th>W</th><th>L</th><th>PCT</th></tr></thead><tbody>${standings.length?standings.map(teamRow).join(''):'<tr><td colspan="5">No current-season standings yet.</td></tr>'}</tbody></table>${standings.length?'<button type="button" class="btn ghost tswFullStandings" id="tswFullStandings">View full standings ↗</button>':''}</aside></main>
    </div>`;
    ...
    $('#tswSeason').onchange=e=>{STATE.tswSeason=e.target.value;renderTeam();};
    ...
    if(standings.length)$('#tswFullStandings').onclick=()=>openStandings(L,t);
  };
